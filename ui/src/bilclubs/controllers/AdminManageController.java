    ...
    public void initialize() throws IOException {
        ...
        new Thread(() -> {
            try {
                ...
                final JSONArray allClubsJSONArray = allClubsResponse.getPayload().optJSONArray("clubs") != null
                        ? allClubsResponse.getPayload().getJSONArray("clubs")
                        : new JSONArray();
                ...
                JSONusers.put("action", "listUsers");
                JSONusers.put("userId", Controller.userId);
                JSONusers.put("sessionToken", Controller.sessionToken);

                Response userListResponse = RequestManager.sendPostRequest("api/user", JSONusers);
                if (userListResponse.getCode() != 200) {
                    System.out.println("listUsers returned " + userListResponse.getCode() + " with error: "
                            + userListResponse.getErrorMessage());
                } else {
                    System.out.println("listUsers fetched successfully.");
                }

                JSONArray members = userListResponse.getPayload().optJSONArray("users");
                if (members != null) {
                ...
                final JSONArray eventList = eventsResponse.getPayload().optJSONArray("events") != null
                        ? eventsResponse.getPayload().getJSONArray("events")
                        : new JSONArray();
                    ...
                    try {
                        ...
                        for (int i = 0; i < membersToRender.length(); i++) {
                            ...
                            try {
                                ...
                                System.out.println("Failed to construct MemberCard!");
                                e.printStackTrace();
                            }
                        }
